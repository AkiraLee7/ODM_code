...
def main():
    ...
    Filename3 = 'Result/CNN_Acc_100_30_04_Noise.txt'
    ...
    test_x_noise = np.zeros(test_images.shape)

    train_images, test_x = train_x / 255.0, test_x / 255.0
    ...
    for i in range(30):
        ...
        for j in range(31):
            for k in range(test_x_noise.shape[0]):
                test_x_noise[k,:,:,:] = addsalt_pepper(test_images[k,:,:,:], j/100.0)
            test_x_noise = test_x_noise / 255.0
            test_loss, test_acc = model.evaluate(test_x_noise,  test_labels, verbose=2)
        ...
        print(test_acc)
    file.close()
    print('CNN_Test_100_30_04_Noise Over !')
