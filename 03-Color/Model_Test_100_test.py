    ...
    for c in range(k):
        for i in range(m - 1):
            for j in range(n - 1):
                ...
                if diff1 <= threshold and diff2 > threshold and diff3 > threshold and diff4 > threshold:
                    count_0[i,j,c] = 1
                    count[0] += 1
                    count[6] += 1
                elif diff1 > threshold and diff2 <= threshold and diff3 > threshold and diff4 > threshold:
                    count_2[i,j,c] = 1
                    count[2] += 1
                    count[7] += 1
                elif diff1 > threshold and diff2 > threshold and diff3 > threshold and diff4 <= threshold:
